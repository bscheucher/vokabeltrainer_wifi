...
public class SemFeldUebungsDialog2 extends Dialog<ButtonType>{
	
	/*
	 * (in der Main-App gewähltes) SemantischesFeld-Objekt wird für den Gebrauch in der
	 * gesamten Klasse deklariert und später im Konstruktor initialisiert
	 */
	SemantischesFeld semFeld;
	
	//VokabelnSemFeld-Objekt wird erzeugt, um Vokabeln aus der DB auszulesen und zu speichern
	VokabelnSemFeld vs;
	 ...
	 */
	ArrayList<Vokabel> alVokabelnSemFeld = new ArrayList<>();
 ...
 // Font für die Darstellung chinesischer Schriftzeichen wird initialisiert
    ...
    /*
	 * Der Konstruktor initialisiert den modalen Dialog, indem er ein SemantischesFeld-Objekt
	 ...
	 */
	public SemFeldUebungsDialog2(SemantischesFeld semFeld) {
		
		// SemantischesFeld-Objekt wird initialisiert
		this.semFeld = semFeld;
		
		/*
		 * Die mit dem semantischen Feld verbundenen Vokabel-Elemente werden aus der Datenbank
		 * ausgelesen und in der Array-List alVokabelnSemFeld gespeichert. Hierfür wird
		 * die REST_API-GET-Methode benutzt.
		 */
		try {
			String line = ServiceFunctions.get("vokabelnsemfeld", String.valueOf(semFeld.getSemFeldID()));
			vs = new VokabelnSemFeld(line);
			for(Vokabel v : vs.getVokabelnSemFeld())
				alVokabelnSemFeld.add(v);
		}
		 ...
		 * "Distraktoren" aus der Array-List alVokabelnSemFeld geholt und nach dem
        ...
        btnUeberpruefen.setOnAction(new EventHandler<ActionEvent>() {
            ...
            public void handle(ActionEvent event) {
                ...
                if (auswahl.equals(vokabel.getBedeutung())) {
                    ...
                    if (alKorrektGeloest.size() == alVokabelnSemFeld.size()) {
                    	showFeedbackDialog("Alle Aufgaben in diesem semantischen Feld sind gelöst.\n" +
                                "Statistik:\n" +
                                "Gesamte Versuche: " + totalGuesses + "\n" +
                                "Richtige Versuche: " + correctGuesses + "\n" +
                                "Falsche Versuche: " + incorrectGuesses);
                        getDialogPane().getScene().getWindow().hide(); // Nach Lösung aller Aufgaben wird Dialogfenster geschlossen.
                        
                    } else {
                        setTask(); // Sollten nicht alle Vokabeln korrekt gelöst sein, wird eine neue Aufgabe generiert.
                    }
                } else {
	...
	private void setVokabel() {
		...
		// alUngeloesteVokabeln wird mit Vokabeln gefüllt, die noch nicht in alKorrektGeloest enthalten sind
    ...
    private void setDistraktor1() {
		ArrayList<Vokabel> potenzielleDistraktoren = new ArrayList<>(alVokabelnSemFeld);
		potenzielleDistraktoren.remove(vokabel); // korrekte Lösung wird entfernt
	...
	private void setDistraktor2() {
		ArrayList<Vokabel> potenzielleDistraktoren = new ArrayList<>(alVokabelnSemFeld);
		potenzielleDistraktoren.remove(vokabel); // korrekte Lösung wird eintfernt
		if (distraktor1 != null) {
